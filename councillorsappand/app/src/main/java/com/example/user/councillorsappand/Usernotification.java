...
public class Usernotification extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        li=(ListView)findViewById( R.id.list3);
        SharedPreferences sh= PreferenceManager.getDefaultSharedPreferences(getApplicationContext());

        String hu = sh.getString("ip", "");
        String url = "http://" + hu + ":5000/and_usernotification";
...
//                                rating=new String[js1.length()];
                                ...
                                for(int i=0;i<js.length();i++)
                                {
                                    ...
                                    sub[i]=u.getString("subject");//dbcolumn name
...
//                                gv.setAdapter(new Custom_view_visited_game(getApplicationContext(),name,gamecode));
                                li.setAdapter(new custom_usernotification(getApplicationContext(),sub,des,date,photo));//custom_view_service.xml and li is the listview object
                            }
